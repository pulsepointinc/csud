...
...
"dependencies": {
    "csud": "git+ssh://git@github.com:pulsepointinc/csud.git#1.1.1"
  }
...
```
alternatively, run:
```
npm install git+ssh://git@github.com:pulsepointinc/csud.git#1.1.1
```
...
Several alternative visitor matching schemes use JSONP instead of the iframe-per-partner approach taken by VM2 to implement similar functionality.  Like partner iframes used by csud, JSONP can be cached on user browsers, reducing the amount of network requests necessary to obtain user data.  Unlike partner iframes though,
